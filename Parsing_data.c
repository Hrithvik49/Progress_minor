...
{
	char sno[500];
	char Merchant_id[500];
	char Average_Amount_or_transaction_per_day[500];
	char Transaction_amount[500];
    char Is_declined[500];
    char Total_Number_of_declines_per_day[500];
    char isForeignTransaction[500];
    char isHighRiskCountry[500];
    char Daily_chargeback_avg_amt[500];
    char Six_month_avg_chbk_amt[500];
    char Six_month_chbk_freq[500];
    char isFradulent[500];            // and so on for required no. of columns.
}dict;

int main()
{
	// Initialize the file pointers
	FILE *fp = fopen("F:\\College Study Material\\Minor\\Dataset\\CreditCardOutput.csv", "r"); //open in read mode
	if (!fp)
	{
		printf("error occured");
		return 0;
	}
	char buff[1024*1024];      								  // stores the first 1024*1024 lines into buff
	int row_count = 0;
	int field_count = 0;
...
void printValues(dict values[])
{
	for(int i =0; i<999; i++)
	{
		printf("Sno -> %s, Merchant_id -> %s, Average_Amount_or_transaction_per_day -> %s, Transaction_amount -> %s, Is_declined -> %s, Total_Number_of_declines_per_day -> %s, isForeignTransaction -> %s, isHighRiskCountry -> %s, Daily_chargeback_avg_amt -> %s, Six-month_avg_chbk_amt -> %s, Six-month_chbk_freq -> %s, isFradulent -> %s", values[i].sno, values[i].Merchant_id, values[i].Average_Amount_or_transaction_per_day, values[i].Transaction_amount, values[i].Is_declined, values[i].Total_Number_of_declines_per_day, values[i].isForeignTransaction, values[i].isHighRiskCountry, values[i].Daily_chargeback_avg_amt, values[i].Six_month_avg_chbk_amt, values[i].Six_month_chbk_freq, values[i].isFradulent);
	}
}
